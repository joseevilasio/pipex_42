...
/*   Updated: 2024/08/12 20:50:00 by joneves-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_parent_process(int *fds, pid_t pid, char *str)
{
	if (ft_strncmp("here_doc", str, 8) == 0)
	{
		waitpid(pid, NULL, 0);
	}
	close(fds[1]);
...
int	main(int argc, char **argv, char **envp)
{
	...
	while (cmds[i].args)
	{
		...
		ft_parent_process(fds, pid[i], argv[1]);
		i++;
	}
